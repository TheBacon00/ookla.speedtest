...
Item {
  ...
  function startPhase(nextPhase) {
    ...
    phase = nextPhase
    stderrText = ""
    var scriptPath = Qt.resolvedUrl("omarchy-network-speedtest").toString().replace("file://", "")
    speedTestProc.command = ["bash", scriptPath, nextPhase]
  ...
  SpeedTestOverlay {
    ...
    leftLabel: "DOWNLOAD"
    rightLabel: "UPLOAD"
    runAgainTooltip: "Measure again via speedtest.net"
